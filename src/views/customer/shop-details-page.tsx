              ...
              <div className="rounded-3xl border border-border p-5" key={staff.id}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <Badge className={staff.available ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-red-500/10 text-red-500 border-red-500/20"}>
                      {staff.available ? "Available" : "Away"}
                    </Badge>
                    <Badge className={staff.status === "BUSY" ? "bg-orange-500/10 text-orange-500 border-orange-500/20" : "bg-blue-500/10 text-blue-500 border-blue-500/20"}>
                      {staff.status === "BUSY" ? "On Work" : "Idle"}
                    </Badge>
                  </div>
                </div>
                    ...
                    <div className="flex items-center justify-between rounded-2xl bg-secondary/60 px-4 py-3 group/service" key={assignment.id}>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        {assignment.serviceId && (
                          <Link 
                            href={`/app/booking?shopId=${shopId}&staffId=${staff.id}&serviceId=${assignment.serviceId}`}
                          >
              ...
              <div className="rounded-[2rem] bg-white/5 border border-white/5 p-6 hover:bg-white/10 transition-all" key={review.id}>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-[10px] text-white/30 uppercase tracking-widest mt-0.5">Reviewed {review.staffName}</p>
                  </div>
